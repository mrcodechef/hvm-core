...
fn dec_bits() {
  ...
  assert_debug_snapshot!(rnet.get_rewrites().total(), @"180067");
}
...
fn dec_bits_tree() {
  ...
  if cfg!(feature = "cuda") {
    ...
    assert_debug_snapshot!(rnet.get_rewrites().total(), @"2878529");
  } else {
    ...
    assert_debug_snapshot!(rnet.get_rewrites().total(), @"2875909");
  }
}
...
fn test_church_mul() {
  ...
  if cfg!(feature = "cuda") {
    ...
    assert_debug_snapshot!(rnet.get_rewrites().total(), @"15");
  } else {
...
fn alloc_big_tree() {
  ...
  assert_debug_snapshot!(rnet.get_rewrites().total(), @"24628");
}
...
fn test_neg_fusion() {
  ...
  // TODO: investigate why this difference exists
  if cfg!(feature = "cuda") {
    assert_debug_snapshot!(rnet.get_rewrites().total(), @"160");
  } else {
    assert_debug_snapshot!(rnet.get_rewrites().total(), @"153");
  }
}
